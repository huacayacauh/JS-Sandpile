...
//
// [7.6] construct "P3 (rhomb) Sun by subst" tiling by substitution
// Penrose P3 with no Fat-to-Fat neighborhood:
// * fat tiles are adjacent to thin tiles only,
// * thin tiles are adjacent to fat and thin tiles.
// it is not a symmetric neighborhood
// 
Tiling.P3sunbysubstNeighNoFatFat = function({iterations}={}){
  var tiles = [];
  // push base "sun" tiling
  for(var i=0; i<5; i++){
    // construct tiles
  ...
  // remove some neighbors to get no-fat-fat neighborhood
  console.log("remove some neighbors (fat-thin)");
  tiles.forEach(tile => {
    let newneighbors = tile.neighbors.filter(nid => {
      if(nid === undefined) return false;
      return !(nid[0] == 'fat' && tile.id[0] == 'fat');
    });
    tile.neighbors = newneighbors;
  });
  // construct tiling
...
//
// [7.7] construct "P3 (rhomb) Sun by subst" tiling by substitution
// Penrose P3 with no Thin-to-Thin neighborhood:
// * fat tiles are adjacent to fat and thin tiles.
// * thin tiles are adjacent to fat tiles only,
// it is not a symmetric neighborhood
// 
Tiling.P3sunbysubstNeighNoThinThin = function({iterations}={}){
  var tiles = [];
  // push base "sun" tiling
  for(var i=0; i<5; i++){
    // construct tiles
  ...
  tiles.forEach(tile => {
    let newneighbors = tile.neighbors.filter(nid => {
      if(nid === undefined) return false;
      return !(nid[0] == 'thin' && tile.id[0] == 'thin');
    });
    tile.neighbors = newneighbors;
  });
  // construct tiling
